...
struct WebMetadataAdminController: AdminListController, AdminDetailController, AdminUpdateController {
...
//            .init("model"),
    ...
    func listCells(for model: DatabaseModel) -> [CellContext] {
        [
            .init(model.title, link: LinkContext(label: model.title ?? "Details", permission: Web.Metadata.permission(for: .detail).key)),
//            .init(model.module),
//            .init(model.model),
        ]
    }
