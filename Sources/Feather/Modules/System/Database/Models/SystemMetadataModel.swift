...
extension SystemMetadataModel {
    ...
    static func findAll(on db: Database, filter: ((QueryBuilder<SystemMetadataModel>) -> QueryBuilder<SystemMetadataModel>)? = nil) async throws -> [SystemMetadataModel] {
        var qb = SystemMetadataModel.query(on: db)
...
extension FeatherDatabaseModel where Self: MetadataRepresentable {
    ...
    static func constructMetadataModel(for id: UUID, using input: FeatherMetadata) -> SystemMetadataModel {
    ...
    static func queryMetadataBy(id: UUID, on db: Database) async throws -> QueryBuilder<SystemMetadataModel> {
        SystemMetadataModel.query(on: db)
            .filter(\.$module == Module.uniqueKey)
            .filter(\.$model == Self.uniqueKey)
            .filter(\.$reference == id)
    }

    static func findMetadataBy(id: UUID, on db: Database) async throws -> SystemMetadataModel? {
